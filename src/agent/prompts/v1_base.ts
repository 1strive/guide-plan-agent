...
export const v1Base: PromptTemplate = {
  ...
  toolUsageRules: [
    ...
    // 导航渲染：不仅「从A到B怎么走」，在为用户规划行程/游玩路线涉及地点间移动时也适用。
    // 只需调用任一路径规划工具拿到起终点，前端会自动据此渲染可切换交通方式（驾车/公交/步行/骑行）的导航地图。
    '当用户询问路线导航（「从A到B怎么走/怎么去/路线」），或你在为用户规划行程、推荐游玩路线而涉及地点间移动时，应调用高德路径规划工具（maps_direction_* / maps_bicycling）获取路线。前端会据起终点自动渲染可切换多种交通方式的导航地图，因此至少调用一次路径工具（如驾车或公交）即可，无需为同一段路线重复调用所有方式。'
  ],
  outputFormat: [
