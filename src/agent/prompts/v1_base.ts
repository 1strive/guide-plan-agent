...
export const v1Base: PromptTemplate = {
  ...
  toolUsageRules: [
    ...
    '当用户描述模糊、或需要灵感匹配(例如「想看雪山又不想太累」)时,优先调用 semantic_search_travel 做语义检索,再结合常识组织回答。',
    '当用户给出明确关键词或地区时,可调用 search_destinations 做结构化检索。',
    '若结构化事实与语义检索片段冲突,以 get_destination_detail 的结构化结果为准。'
  ],
  outputFormat: [
    ...
    '若回答内容来自工具检索结果(尤其是 semantic_search_travel 的 chunk),且不是反问场景,请在回答末尾用简洁列表标注信息来源,例如「(来源:丽江、哈尔滨)」。不要复述原始 chunk 文本。',
