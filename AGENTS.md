...
`docs/03-开发笔记/` 是每个开发阶段的**代码改动速查手册**,目的是让未来回看时能直接跳到对应文件,不用逆向猜实现。每完成一个阶段(或一批关联 Task)必须补齐对应 `note-NN-阶段N-xxx.md`。

### 笔记必须包含的两类映射

**(1) 代码实现映射**:每个 Task 必须明确列出:
- **改了哪些文件**:区分"新增 / 改代码 / 改 schema / 改前端 / 改文档"
- **核心代码位置**:给出文件路径 + 函数/类名 + 关键行号或行号范围(如 `runManager.ts:start()`)
- **关键代码片段**:贴出最核心的 10~30 行代码(签名 / 数据结构 / 关键判断),不需要全文
- **设计决策与边界**:为什么这么做、不做什么、已知限制

**(2) 八股文映射**:每个 Task 必须明确关联到 `docs/01-面试八股文/` 的具体章节:
- 格式:`八股:NN-文件名.md §章号 小节名`
- 一个 Task 可能关联多个八股章节(用列表列出)
- 如果是该八股考点在项目中的**首次工程落地**,特别标注"首次实践"
...
> 本文目标:...
> 配套文档:架构总图 ... ;实验数据 ...
> 覆盖范围:Task X.Y ✅ / Task X.Z ✅

## 总览(表格:Task → 一句话 → 新增文件 → 改动文件)
