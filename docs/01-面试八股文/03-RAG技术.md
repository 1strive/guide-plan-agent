...
> **本项目实践说明(2026-06-01)**:本项目最终**未采用静态 RAG 架构**。原因:旅游场景对实时性要求高(开园时间、票价、活动、天气随时变),静态 RAG(seed → chunk → 向量库 → 召回)不适合,改为**阶段4 通过 MCP/Skills 调用 Tavily web_search / 高德地图 / 和风天气等外部工具动态获取数据**——本质上是"动态 RAG"(查询时拉数据,不维护离线索引)。
>
> 完整决策(包含"完整实施过 Task 3.1~3.6 → 实测后判定不适合 → 回滚"的工程判断 STAR 故事)见 `docs/开发规划.md` 关键设计决策 #6 + `docs/04-架构文档/agent-架构.md` §5.9。
>
> **本文作为面试知识点学习材料保留**,代码层面与本项目无对应。面试讲法:学了 RAG 八股 → 实施了 → 实测后判定不适合 → 改用动态 RAG。
