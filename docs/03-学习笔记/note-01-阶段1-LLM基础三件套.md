...
**问题驱动**:你处理"今天**天气**真好,我们去公园吧"这句话,翻译成英文需要知道"天气"和"weather"对应——但同时,"公园"也得知道前面有"我们",才不会翻成"a park"而是"the park"。

**RNN/LSTM 时代**(2014~2017):
- 按时间步从左往右处理,一个词一个词读
- **问题1**:长距离依赖丢失——句子开头的信息到末尾就模糊了
- **问题2**:**串行计算**,不能并行,训练慢

**Transformer 的突破**(2017,《Attention is All You Need》):
- 抛弃循环结构,**所有词同时被处理**(并行!)
- 用 **Attention** 机制让任意两个词直接"对话",距离不再是问题
- 训练速度数量级提升 → 才有了 GPT-3 / Claude / Llama 这一代大模型

> **类比**:RNN 像接力赛传话,一个传一个,后面的人对前面的话越来越模糊。Transformer 像全员开 Zoom,每个人都能直接和任何人对话,信息不衰减。
