...
> **使用建议**:每道题先盖住答案、用自己的话默答一遍,再展开比对——这才是"输出倒逼输入"的学习闭环。直接读答案的吸收率不到一半。

### Transformer / Self-Attention

**1. RNN 处理长序列有什么问题,Transformer 怎么解决的?**

> **答**:RNN 两大致命伤——① 串行计算(`memory_t` 必须等 `memory_{t-1}`),GPU 用不上;② 梯度沿时间步连乘,20+ 步就消失/爆炸,句首信息传不到句尾。Transformer 用 Self-Attention 让任意两 token 一步直接交互,长距离 1 步可达,且整层可并行算。

**2. Self-Attention 的 Q/K/V 分别是什么角色?**

> **答**:类比 SQL——Q(Query)是 WHERE 条件「我在找什么」;K(Key)是索引列「我能被谁找到」;V(Value)是 SELECT 列「被找到后我贡献什么」。每个 token 用自己的 Q 去匹配所有 token 的 K 拿到权重,再加权求和所有 V,得到带上下文的新表示。

**3. 为什么需要 Multi-Head Attention?**

> **答**:一个 Attention 头只能学一种关系模式。Multi-Head 并行跑 N 个独立 Attention(GPT-3 是 96 头),每头各管一类——语法依赖、指代消解、语义关联……最后拼起来,信息更丰富、鲁棒性更好。

**4. Self-Attention 的时间复杂度是 O(?),这对工程有什么影响?**

> **答**:**O(n²)**(n = 序列长度)。工程后果:① 输入翻倍计算量四倍,响应延迟和输入长度强相关;② 上下文窗口有硬上限(128k 不只是训练限制,更是 attention 显存);③ 催生了 **KV Cache**(已生成 token 的 K/V 缓存复用)、Flash Attention 等优化。

**5. 为什么超长上下文会出现"Lost in the middle"?**

> **答**:经验现象——模型对开头和结尾信息利用率高、中段权重不足。常见归因:① 训练语料里关键信息常落在首尾;② 长序列下 attention 注意力分布趋平。工程对策:重要信息放头尾、长文档先 RAG 召回再喂模型,而不是塞全文。

### Token / Tokenizer

**6. Token、字符、单词三者的关系?**

> **答**:Token 是 Tokenizer 切出的**亚词单元(subword)**,介于字符和单词之间。高频词常 1 token(`the`、`你好`),低频词被拆(`unbelievable` = `un`+`believ`+`able`,3 token),没见过的回退到字节。所以「1 token 多少字符」没有定值,**Token ≠ 字符 ≠ 单词**。

**7. BPE 算法的核心思想?**

> **答**:**迭代合并最高频字符对**——① 初始把所有字符当 token;② 统计语料里最常出现的字符对;③ 合并成新 token;④ 重复直到词表达到目标大小(GPT-3.5/4 ~100k)。结果是高频词单 token、低频词被拆、未见过的用字节兜底。

**8. 为什么中文比英文 token 更密?**

> **答**:英文 1 token ≈ 4 字符(常见单词整词成 token),中文 1 token ≈ 1~1.5 字符(汉字密度高,常见字单独成 token)。**同字数下中文成本约英文 2~3 倍**。设计 prompt 时英文 system prompt 更省 token,但用户 query 是中文无法优化。

**9. 为什么输出 token 比输入贵?**

> **答**:输入是「一次性编码读完」,输出是「逐 token 自回归生成」——每生成 1 token 都要跑一遍完整前向 + KV Cache 增长。生成比读取更耗算力,所以输出常贵 2~5 倍(GPT-4o-mini 输出价是输入 4 倍)。

**10. 我们项目的 `estimateTokens` 为什么用 `length / 2`,什么时候应该升级为 tiktoken?**

> **答**:`length / 2` 是中英混合下的**保守上界**(中文 1 字符≈1 token、英文≈0.25,取 0.5 偏保守),**仅在 API 不返回 usage 时兜底**(部分 MiniMax 兼容协议不实现 `stream_options.include_usage`)。升级时机:阶段5 网关层做语义缓存命中/限流/精细计费时接 tiktoken 拿真值——代价是 +30KB 依赖,且不同模型的 tokenizer 不通用(GPT 和 Claude 各自一套)。

### 推理参数

**11. T=0 等于完全确定吗?为什么?**

> **答**:**不等于**。① 浮点数 + GPU 并行加法的结合律不严格成立,同样输入在不同 batch / GPU 上有微小差异;② 部分 API 内部仍有最低采样温度。严格复现要 **T=0 + 固定 `seed` + 固定模型版本** 三件套齐全。

**12. T 和 top_p 哪个改"形状"、哪个改"范围"?**

> **答**:**T 改形状**——logits 除以 T 再 softmax,T 低柱子高低差异大(分布尖)、T 高变平。**top_p 改范围**——抽样前只保留累积概率达 P 的 top tokens,长尾全砍,留几根柱子。

**13. 工业上为什么 T 和 top_p 二选一调?**

> **答**:两者交互非线性,双调容易出现「高 T 配低 top_p 看着稳定其实在窄范围里乱跳」这类难以预测的组合效应。约定俗成:挑一个调(常调 T),另一个保留默认(top_p=1)。我们项目就是只调 T(0.4)、top_p 保持 1。

**14. max_tokens 的两个作用?为什么 JSON 输出不能只靠 max_tokens 限制?**

> **答**:两个作用——① **成本保护**:防陷入循环或冗长输出;② **延迟保护**:首字延迟低 ≠ 总延迟低,生成 2000 token 可能 30s+。JSON 不能只靠它:截断时不会「漂亮地结束」,可能切在 JSON 中间产生非法 JSON。正确做法是用 **function calling / 结构化输出**,让模型按 schema 收尾,max_tokens 只兜底。

**15. 旅游 Agent 场景为什么 T 选 0.4 而不是 0.0 或 0.7?**

> **答**:旅游推荐要「**既靠谱又有趣**」——T=0 推荐千篇一律(同问题永远同答案,失去顾问感),还会让模型错得很自信;T=0.7 适合创意写作,但工具调用 JSON 参数稳定性下降。0.4 是平衡点:多样性够用、事实表述稳定、工具调用解析失败率低,且阶段2 Task 2.3 评测时方便和 0.2 / 0.7 做对照实验。

> **延伸练习**:把这 15 道当首轮筛子,**仍然答不顺**的题翻 `docs/01-面试八股文/07-大模型基础.md` 精读对应章节,然后过两天再默答一次——间隔重复是把知识从「认得」变「会讲」的关键。
