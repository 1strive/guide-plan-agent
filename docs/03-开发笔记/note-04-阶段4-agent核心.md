...
```json
{
  "level": 30,
  "runId": "0cacd05c-...",
  "tool": "get_destination_detail",
  "toolCallId": "019e83ad-...",
  "durationMs": 14,
  "argsPreview": "{\"destination_id\":2}",
  "resultPreview": "{\"error\":\"destination not found\",\"destination_id\":2}",
  "msg": "tool finished"
}
```
...
事件自动持久化到 Redis Stream + archived_run_events(runManager pump 不挑事件类型),续订时回放。前端不消费(switch fall through),后续 web 迭代可加 UI。
...
| **不改 schema** | `archived_run_events` 表                                              | 4.1 + 4.2             | 新事件 JSON 直接落 `event_json`，无 migration                                                                                                                                          |
...
| 存储     | Redis Stream + `archived_run_events(run_id, seq, event_json)` | `chat_sessions.summary`     |
...
- `eval/testset.ts` 删除 `keyword_search`/`detail_list` category 和 kw-_/detail-_ case
