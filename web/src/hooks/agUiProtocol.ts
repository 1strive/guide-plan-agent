...
// ─── 内置协议处理器注册（与 note-05-AG-UI协议文档.md 事件列表对齐）───

registerProtocolHandler('RUN_STARTED', (state, event) => {
    state.runId = event.runId as string
})

registerProtocolHandler('TEXT_MESSAGE_CONTENT', (state, event) => {
    state.content += event.delta as string
})

registerProtocolHandler('THINKING_CONTENT', (state, event) => {
    state.thinking += event.delta as string
})

registerProtocolHandler('TOOL_CALL_START', (state, event) => {
    state.toolCalls.push({ name: event.toolCallName as string, status: 'running' })
})

registerProtocolHandler('TOOL_CALL_END', (state) => {
    const running = state.toolCalls.find((t) => t.status === 'running')
    if (running) running.status = 'done'
})

registerProtocolHandler('RUN_ERROR', (state, event) => {
    state.content += `\n[错误] ${event.message}`
})

/** ASK_USER：一等中断事件，直接携带问题数据 */
registerProtocolHandler('ASK_USER', (state, event) => {
...
registerProtocolHandler('RUN_FINISHED', (state, event) => {
    if (state.interrupt) return
    const outcome = event.outcome as { type: string; interrupts?: RawInterrupt[] } | undefined
